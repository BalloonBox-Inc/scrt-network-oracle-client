...
const REDIRECT_URL = `${process.env.NEXT_BASE_URL}/applicant/generate?type=coinbase&status=loading`;
  ...
  if (req.query?.code) {
    try {
      const tokenRes = await fetch(
        `${COINBASE_TOKEN_URL}?grant_type=authorization_code&code=${req.query.code}&client_id=${CLIENT_ID}&client_secret=${COINBASE_CLIENT_SECRET}&redirect_uri=${process.env.NEXT_BASE_URL}/applicant/generate?type=coinbase&status=loading`,
        {
          method: 'POST',
          headers: {
